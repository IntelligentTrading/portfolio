...
const ctrl = (module.exports = {
  ...
  rebalanceUser: async user => {
    if (user) {
      ...
      if (
        (user.portfolio.custom == null || user.portfolio.custom.length === 0) &&
        user.portfolio.packs.length <= 0
      ) {
        return Promise.resolve({
          statusCode: 500,
      ...
      return packsCtrl.list().then(dbpacks => {
        ...
        if (user.portfolio.custom != null && user.portfolio.custom.length > 0) {
          userPacks = [{ allocations: user.portfolio.custom }];
        }
  ...
  packs: {
    ...
    custom: async (id, allocations) => {
      let message = "";
      return ctrl.getById(id).then(user => {
        if (user) {
          user.portfolio.custom = allocations;

          message = "Custom package applied.";
          user.save();
          return { statusCode: 200, object: message };
        }
        return { statusCode: 418, object: "User not found" };
